...
import { Menu, X, Bell, LogOut, Settings, User, ChevronDown, Shield, GraduationCap, Mic2, Crown, CheckCheck, Inbox } from "lucide-react";
...
export function Navbar() {
  ...
  const { user, session, isLoggedIn, logout } = useAuth();
  ...
  const handleNotifClick = (notifId: string, actionUrl: string | null) => {
    markOneRead.mutate(notifId);
    if (actionUrl) {
      navigate(actionUrl);
    }
  };
